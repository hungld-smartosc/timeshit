  ...
  async function getLoggedDay(CSRF_TOKEN, beginningDate) {
    ...
    const response = await fetch(
      "https://people.zoho.com/hrportal1524046581683/viewAction.zp",
      {
        ...
        body: `mode=fetchRecords&formId=${pcIdLogTimeSheet}&viewId=${viewId}&isOnload=true&typeOfView=mydep&startInd=1&limit=${dayDiff(beginningDate)}&conreqcsr=${CSRF_TOKEN}`,
        method: "POST",
      }
    );
    const {
