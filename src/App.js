...
async function fetchTasksAndProjects(CSRF_TOKEN) {
  const response = await fetch(
    "https://people.zoho.com/hrportal1524046581683/formAction.zp",
    {
      ...
      body: `formId=412762000158024844&mode=getFormComponents&subMode=ADD&conreqcsr=${CSRF_TOKEN}`,
      method: "POST",
    }
  );
  const {
...
async function getLoggedDay(CSRF_TOKEN) {
  const response = await fetch(
    "https://people.zoho.com/hrportal1524046581683/viewAction.zp",
    {
      ...
      body: `mode=fetchRecords&formId=412762000158024844&viewId=412762000158024846&isOnload=true&typeOfView=self&startInd=1&limit=30&conreqcsr=${CSRF_TOKEN}`,
      method: "POST",
    }
  );
  const {
